...
export default function HomePage() {
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-[#0b0d0e] text-white">
      <div className="pointer-events-none absolute inset-x-0 top-0 h-[520px] bg-[radial-gradient(circle_at_78%_18%,rgba(143,167,184,0.18),transparent_32%),radial-gradient(circle_at_18%_0%,rgba(255,255,255,0.08),transparent_25%)]" />
      ...
      <div className="relative mx-auto flex min-h-screen max-w-7xl flex-col px-5 sm:px-8 lg:px-12">
        <header className="flex items-center justify-between border-b border-white/10 py-5">
          ...
          <a
            ...
            className="rounded-full border border-white/15 px-4 py-2 text-sm font-bold transition hover:border-[#8fa7b8] hover:text-[#b9ccd9]"
          >
            (929) 264-2629
          </a>
        </header>
        ...
        <section className="grid flex-1 items-center gap-14 py-14 lg:grid-cols-[1.08fr_0.92fr] lg:py-20">
          <div>
            <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-[#8fa7b8]/30 bg-[#8fa7b8]/10 px-4 py-2 text-xs font-bold uppercase tracking-[0.18em] text-[#b9ccd9]">
              <span className="h-2 w-2 animate-pulse rounded-full bg-[#8fa7b8]" />
              Customer tracking portal
            </div>

            <h1 className="max-w-3xl text-5xl font-black leading-[0.94] tracking-[-0.055em] sm:text-6xl lg:text-7xl">
              Your vehicle.
              <span className="block text-[#9fb5c5]">Your route.</span>
              One clear update.
            </h1>
            ...
            <div className="mt-8 flex flex-wrap gap-x-7 gap-y-3 text-sm font-semibold text-white/55">
              <span className="flex items-center gap-2"><span className="text-[#8fa7b8]">✓</span> Private tracking number</span>
              <span className="flex items-center gap-2"><span className="text-[#8fa7b8]">✓</span> No account required</span>
              <span className="flex items-center gap-2"><span className="text-[#8fa7b8]">✓</span> Mobile friendly</span>
            </div>
          </div>

          <div className="relative">
            <div className="absolute -inset-8 rounded-[3rem] bg-[#8fa7b8]/10 blur-3xl" />
            <div className="relative rounded-[2rem] border border-white/12 bg-white/[0.065] p-6 shadow-2xl backdrop-blur sm:p-8">
              <p className="text-xs font-black uppercase tracking-[0.2em] text-[#9fb5c5]">Track a transport</p>
              ...
              <form onSubmit={trackTransport} className="mt-7">
                ...
                <div className="mt-2 rounded-2xl border border-white/15 bg-black/30 p-2 focus-within:border-[#8fa7b8] focus-within:ring-4 focus-within:ring-[#8fa7b8]/10 sm:flex">
                  <input
                    id="tracking-number"
                ...
                {error ? (
                  <p id="tracking-error" className="mt-3 text-sm font-semibold text-[#b9ccd9]" role="alert">{error}</p>
                ) : (
              ...
              <div className="mt-8 grid gap-3 sm:grid-cols-3">
                {features.map((feature) => (
                  <div key={feature.number} className="rounded-2xl border border-white/8 bg-black/20 p-4">
                    <p className="text-xs font-black text-[#9fb5c5]">{feature.number}</p>
        ...
        <footer className="flex flex-col gap-3 border-t border-white/10 py-6 text-xs text-white/35 sm:flex-row sm:items-center sm:justify-between">
          ...
          <p>Need help? Call or text <a className="font-bold text-white/65 hover:text-[#b9ccd9]" href="tel:+19292642629">(929) 264-2629</a></p>
        </footer>
      </div>
    </main>
  );
